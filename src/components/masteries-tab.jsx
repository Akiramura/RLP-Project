...
export function MasteriesTab({ puuid, profile }) {
    ...
    if (!puuid) {
        return (
            <div className="flex items-center justify-center h-64">
                <p className="text-slate-500">Nessun summoner selezionato.</p>
            </div>
        );
    }
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center h-64 gap-3">
                ...
                <p className="text-slate-400">Caricamento maestrie...</p>
            </div>
        );
    }
    ...
    if (!masteries || masteries.length === 0) {
        return (
            <div className="flex items-center justify-center h-64">
                <p className="text-slate-500">Nessuna maestria trovata.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            {/* Header summary */}
            <Card className="p-5 bg-gradient-to-br from-slate-900 to-slate-800 border-yellow-900/40">
                <div className="flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-2">
                        <Star className="w-6 h-6 text-yellow-400" />
                        <div>
                            <p className="text-white font-bold text-lg">{profile?.gameName ?? "Summoner"}</p>
                            <p className="text-slate-400 text-xs">{masteries.length} campioni con maestria</p>
                        </div>
                    </div>
                    <div className="flex gap-3 ml-auto flex-wrap">
                        <div className="bg-slate-800 rounded-lg px-3 py-2 text-center">
                            ...
                            <p className="text-slate-500 text-xs">Punti totali</p>
                        </div>
                        {m7 > 0 && (
                            <div className="bg-slate-800 rounded-lg px-3 py-2 text-center">
                                <p className="text-yellow-300 font-bold">{m7}</p>
                                <p className="text-slate-500 text-xs">M7</p>
                            </div>
                        )}
                        {m6 > 0 && (
                            <div className="bg-slate-800 rounded-lg px-3 py-2 text-center">
                                <p className="text-purple-300 font-bold">{m6}</p>
                                <p className="text-slate-500 text-xs">M6</p>
                            </div>
                        )}
                        {m5 > 0 && (
                            <div className="bg-slate-800 rounded-lg px-3 py-2 text-center">
                                <p className="text-red-300 font-bold">{m5}</p>
                                <p className="text-slate-500 text-xs">M5</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                {masteries.slice(0, 3).map((m, i) => {
                    ...
                    const lvlColor = MASTERY_COLORS[m.championLevel] ?? "text-slate-400";
                    const sizes = ["w-24 h-24", "w-20 h-20", "w-16 h-16"];
                    return (
                        <Card key={m.championId} className={`p-4 bg-slate-900 border-slate-700 flex flex-col items-center gap-2 ${i === 0 ? "border-yellow-700/50" : ""}`}>
                            <div className="relative">
                                <img
                                    src={`https://ddragon.leagueoflegends.com/cdn/${PATCH}/img/champion/${champName}.png`}
                                    alt={champName}
                                    className={`${sizes[i]} rounded-xl object-cover border-2 ${i === 0 ? "border-yellow-600/60" : "border-slate-700"}`}
                                    onError={e => { e.target.style.display = "none"; }}
                                />
                                <Badge className={`absolute -bottom-2 left-1/2 -translate-x-1/2 ${lvlColor} bg-slate-900 border border-slate-700 text-xs px-1.5`}>
                                    M{m.championLevel}
                                </Badge>
                            </div>
            ...
            <Card className="p-4 bg-slate-900 border-slate-700">
                ...
                <div className="space-y-1 max-h-[520px] overflow-y-auto pr-1">
                    {masteries.map((m, idx) => {
                        const champName = champKeyMap[String(m.championId)] ?? `Champ ${m.championId}`;
                        const lvlColor = MASTERY_COLORS[m.championLevel] ?? "text-slate-500";
                        const topPct = masteries.length > 0 ? (m.championPoints / masteries[0].championPoints) * 100 : 0;
                        return (
                            <div key={m.championId} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-800/60 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full transition-all ${m.championLevel >= 7 ? "bg-yellow-500" : m.championLevel >= 6 ? "bg-purple-500" : m.championLevel >= 5 ? "bg-red-500" : "bg-slate-600"}`}
                                ...
                                <div className="text-right shrink-0 w-20">
                                    ...
                                    {m.championPointsUntilNextLevel > 0 && m.championLevel < 7 && (
                                        <p className="text-slate-600 text-xs">{formatPoints(m.championPointsUntilNextLevel)} to next</p>
                                    )}
                                </div>
                                {m.chestGranted && (
